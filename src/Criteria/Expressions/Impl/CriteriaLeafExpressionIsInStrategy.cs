...
{
	public class CriteriaLeafExpressionIsInStrategy : ICriteriaLeafExpressionStrategy
	{
		private readonly ICriteriaTypeRegistry _typeRegistry;

		public CriteriaLeafExpressionIsInStrategy(ICriteriaTypeRegistry typeRegistry)
		{
		...
		public Expression GetExpression(ICriteriaLeaf leaf)
		{
			...
			if(itemCollection != null)
			{
				...
				result = Expression.Call(typeof(Enumerable), "Contains", new[] { itemType }, new[] { arrayExpression, accessorExpression });
			}
			else
			{
				throw new ArgumentException("Cannot use operator 'IsIn' on non-IEnumerable value.");
			}
